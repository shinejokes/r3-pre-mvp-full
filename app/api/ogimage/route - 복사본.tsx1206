...
// original_url을 보고 간단한 설명 문구를 추론
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from("r3_shares")
    .select("title, views, hop, original_url")
    .eq("ref_code", shareId)
    .maybeSingle();

  const title = (data?.title ?? "R3 링크").slice(0, 50); // 너무 길면 잘라주기
  const views = data?.views ?? 0;
  const hop = data?.hop ?? 0;
  const description = getAutoDescription(data?.original_url);
